...
public class ProductController {
    ...
    public ResponseEntity<?> getAllProducts(Pageable pageable,
        ...
        try {
            ...
            List<ProductDTO> productDTOS = modelMapper.map(
                    products.getContent(), new TypeToken<List<ProductDTO>>(){}.getType()
            );
    ...
    public ResponseEntity<?>  getProductByCategoryId(@PathVariable(value = "categoryId") long categoryId, Pageable pageable){
            try {
                Page<ProductDTO> productDTOS = productService.getProductByCategoryId(categoryId,pageable);
                PagedModel.PageMetadata pageMetadata = new PagedModel.PageMetadata(
                        pageable.getPageSize() , productDTOS.getNumber(), productDTOS.getTotalElements(), productDTOS.getTotalPages()
                );
                PagedModel<ProductDTO> pagedModel = PagedModel.of(productDTOS.getContent(), pageMetadata);
                pagedModel.add(linkTo(methodOn(ProductController.class).getProductByCategoryId(categoryId, pageable)).withSelfRel());
